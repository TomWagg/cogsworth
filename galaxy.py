...
class Galaxy():
    ...
    def D(self):
        if self._D is None:
            R_sun = 8 * u.kpc
            self._D = np.sqrt(self._z**2 + self._R**2 + R_sun**2 - 2 * R_sun * self._R * np.cos(self._theta))
        return self._D
    ...
    def sample(self):
        ...
        self._which_comp = np.concatenate([[self._components[i]] * sizes[i] for i in range(len(self._components))])

        self._tau = [None for _ in range(len(self._components))]
        self._R = [None for _ in range(len(self._components))]
        self._z = [None for _ in range(len(self._components))]

        # go through each component and get lookback time, radius and height
        for i, com in enumerate(self._components):
            self._tau[i] = self.draw_lookback_times(sizes[i], component=com)
            self._R[i] = self.draw_radii(sizes[i], component=com)
            self._z[i] = self.draw_heights(sizes[i], component=com)

        # combine the samples
        self._tau = np.concatenate(self._tau)
        self._R = np.concatenate(self._R)
        self._z = np.concatenate(self._z)
